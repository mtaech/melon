...
const S = {
    ...
    link: { color: "var(--dsw-alias-brand-text)", fontSize: 13, textDecoration: "none", display: "inline-flex", alignItems: "center", gap: 4, marginLeft: "auto" },
...
function Card({ entry, onUpgrade, onUninstall }) {
    ...
    return React.createElement("div", { style: S.card }, React.createElement("div", { style: S.row }, React.createElement("span", { style: S.name }, entry.name), ...badges, React.createElement(Pill, { status: entry.status })), React.createElement("div", { style: S.desc }, entry.description ?? ""), React.createElement("div", { style: S.vers }, React.createElement("span", null, "installed ", React.createElement("b", { style: S.versB }, entry.installedVersion ?? "—")), React.createElement("span", null, "latest ", React.createElement("b", { style: S.versB }, latest))), React.createElement("div", { style: S.actions }, entry.url ? React.createElement("a", { href: entry.url, target: "_blank", rel: "noreferrer", style: S.link, title: entry.url }, "仓库 ↗") : null, button, uninstallBtn));
}
...
function DashboardTab(props) {
    ...
    return React.createElement("div", null, dshRunning ? React.createElement("div", { style: S.banner }, "⚠ dsh 正在运行：升级会改动 profile 的 node_modules，请先停止 dsh 再应用升级。") : null, React.createElement("div", { style: S.header }, React.createElement("strong", null, `插件管理（${visible.length}）`), React.createElement("div", { style: S.searchBox }, React.createElement("input", { style: { ...S.search, border: "none", padding: 0, flex: 1, minWidth: 0, background: "transparent", outline: "none" }, placeholder: "搜索插件…", value: query, onChange: (e) => setQuery(e.target.value) }), query ? React.createElement("button", { style: S.clear, onClick: () => setQuery(""), title: "清除搜索", type: "button" }, "×") : null), React.createElement("button", { style: S.btn, onClick: reload }, "刷新")), loading
        ...
        : error
            ...
            : visible.length === 0
                ...
                : React.createElement("div", null, visible.map((e) => React.createElement(Card, { key: e.name, entry: e, onUpgrade: openUpgrade, onUninstall: openUninstall }))), open ? React.createElement(UpgradeModal, { modal, onClose: close, onApply: doApply }) : null, uninstallState ? React.createElement(UninstallModal, { state: uninstallState, onClose: closeUninstall, onApply: doUninstall }) : null);
}
function UpgradeModal({ modal, onClose, onApply }) {
    const plan = modal.plan;
    const rows = [];
    if (plan) {
        rows.push(["来源", plan.source], ["当前", `${plan.installedVersion ?? "—"}${plan.installedCommit ? ` (${short(plan.installedCommit)})` : ""}`], ["当前版本日期", dateLabel(plan.currentVersionDate)], ["目标", plan.targetLabel], ["最新版本日期", dateLabel(plan.latestVersionDate)], ["旧 specifier", plan.currentSpecifier], ["新 specifier", plan.newSpecifier]);
        if (plan.error)
            rows.push(["错误", plan.error]);
        else
            rows.push(["命令", React.createElement("code", null, plan.command)]);
    }
