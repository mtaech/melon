...
function sameExe(exePath, candidate) {
    if (!candidate)
        return false;
...
export function processesByExecutable(exePath) {
    ...
    if (process.platform === "darwin") {
        ...
        for (const line of output.split("\n")) {
            ...
            if (sameExe(exePath, firstArg))
                out.push({ pid, args: command.split(" ").slice(1) });
        }
        return out;
    }
 ...
 */
export async function terminateProcessTree(pid, graceMs = 2000) {
    const groupPid = -pid;
