...
function markReportedInitFailure(error) {
    error[REPORTED_INIT_FAILURE] = true;
    return error;
}
function isReportedInitFailure(error) {
    return error instanceof Error && error[REPORTED_INIT_FAILURE] === true;
}
async function waitForTabCleanup(tab, timeoutMs, pendingResource, promise) {
...
export function acquireTab(name, browser, opts) {
    ...
    void tail.then(() => {
        if (acquireChains.get(name) === tail)
            acquireChains.delete(name);
    });
    return result;
}
async function acquireTabImpl(name, browser, opts) {
        ...
        if (existing.browser === browser && existing.state === "alive") {
            if (opts.dialogs !== undefined && opts.dialogs !== existing.dialogPolicy) {
                ...
                if (opts.viewport) {
                    ...
                    reuseSteps.push(`await page.setViewport({ width: ${opts.viewport.width}, height: ${opts.viewport.height}, deviceScaleFactor: ${dsf === undefined ? "undefined" : String(dsf)} });`);
                }
                if (opts.url) {
    ...
    try {
        ...
        try {
            ...
            if (tempHold || browser.refCount === 0)
                await releaseBrowser(browser, { kill: false });
            const finalError = new ToolError(`Failed to start browser tab worker (inline fallback also failed): ${inlineError instanceof Error ? inlineError.message : String(inlineError)}`);
...
export async function runInTab(name, opts) {
    return await runInTabWithSnapshot(name, { code: opts.code, timeoutMs: opts.timeoutMs, signal: opts.signal }, { cwd: opts.cwd, browserScreenshotDir: opts.screenshotDir, excludeWebP: opts.excludeWebP });
}
async function runInTabWithSnapshot(name, opts, snapshot) {
    const tab = tabs.get(name);
    if (!tab || tab.state === "dead") {
        const killed = killedTabs.get(name);
        throw new ToolError(killed
    ...
    if (tab.pending.size > 0)
        throw new ToolError(`Tab ${JSON.stringify(name)} is busy`);
    const id = uid.next();
    const { promise, resolve, reject } = Promise.withResolvers();
    const closeAc = new AbortController();
    const pending = { resolve, reject, signal: opts.signal, toolCalls: new Map(), closeAc };
    tab.pending.set(id, pending);
    const abort = () => {
        tab.worker.send({ type: "abort", id });
        for (const ctrl of pending.toolCalls.values())
...
export async function releaseTab(name, opts = {}) {
    const tab = tabs.get(name);
    if (!tab) {
...
export async function releaseAllTabs(opts = {}) {
    const names = [...tabs.keys()];
    let count = 0;
    for (const name of names) {
        if (await releaseTab(name, opts))
            count++;
    }
    return count;
}
export async function dropHeadlessTabs() {
...
export async function releaseTabsForOwner(ownerId, opts = {}) {
    if (!ownerId)
        return 0;
...
export function getTabsMapForTest() {
    return tabs;
}
async function buildInitPayload(browser, opts) {
    const safeDir = getPuppeteerDir();
    const browserWSEndpoint = browser.browser.wsEndpoint();
    if (!browserWSEndpoint)
        throw new ToolError("Browser websocket endpoint is unavailable");
...
async function dispatchToolCall(tab, msg) {
...
async function recycleTimedOutWorkerTab(tab, timeoutMs) {
    ...
    try {
        ...
        try {
            ...
            await worker.terminate().catch(() => undefined);
            const finalError = new ToolError(`Failed to recycle timed-out browser tab worker (inline fallback also failed): ${inlineError instanceof Error ? inlineError.message : String(inlineError)}`);
            Object.defineProperty(finalError, "cause", { value: error, configurable: true });
            throw finalError;
        }
    }
}
async function forceKillTab(name, reason) {
    const tab = tabs.get(name);
    if (!tab)
        return;
...
async function closeTargetById(browser, targetId) {
...
async function closeOrphanTarget(tab) {
    await closeTargetById(tab.browser, tab.targetId);
}
function closeAbandonedWorkerPage(browser, worker) {
...
async function waitForClosed(tab) {
...
async function targetIdForTarget(target) {
    const raw = target;
...
function logWorkerMessage(msg) {
    if (msg.level === "debug")
        logger.debug(msg.msg, msg.meta);
    else if (msg.level === "warn")
        logger.warn(msg.msg, msg.meta);
    else
        logger.error(msg.msg, msg.meta);
}
async function raceWithTimeout(promise, timeoutMs, reason, onTimeout) {
    ...
    try {
        ...
        if (error instanceof ToolError && error.message === reason)
            await onTimeout?.(reason);
        throw error;
    }
    finally {
        clearTimeout(timer);
...
function wrapNodeWorker(worker) {
    return {
        ...
        onError(handler) {
            const onError = (error) => handler(error);
...
async function spawnInlineWorker() {
...
async function initializeTabWorker(worker, payload, timeoutMs, deadlineStart = performance.now()) {
    ...
    const unlisten = worker.onMessage(msg => {
        if (msg.type === "page-created") {
            ...
            ready.resolve(msg.info);
        else if (msg.type === "init-failed")
            failStartup(markReportedInitFailure(errorFromPayload(msg.error)));
...
function initBudgetExhausted(budgetMs, deadlineStart) {
    return budgetMs - Math.round(performance.now() - deadlineStart) <= 0;
}
export function expandBrowserScreenshotDir(raw) {
    return raw ? expandPath(raw) : undefined;
}
export function initializeTabWorkerForTest(worker, payload, timeoutMs, deadlineStart = performance.now()) {
    return initializeTabWorker(worker, payload, timeoutMs, deadlineStart);
}
export function toErrorPayloadForTest(error) {
