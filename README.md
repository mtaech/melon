...
| [`dsh-worksop-plus`](packages/worksop-plus) | Web 插件：接管左侧工作区面板——工作区置顶、自定义分组、隐藏、筛选与批量整理，可一键切回系统面板 |
| [`dsh-browser-notify`](packages/dsh-browser-notify) | Web 插件：agent 回合结束、向你提问或请求授权时，用浏览器系统通知提醒你（页面不在前台时提醒，点击可跳转会话） |
