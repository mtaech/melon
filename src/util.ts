 ...
 */
export function untilAborted<T>(signal: AbortSignal | undefined | null, pr: Promise<T> | (() => Promise<T>)): Promise<T> {
	if (!signal) return typeof pr === "function" ? pr() : pr;
	if (signal.aborted) return Promise.reject(new ToolAbortError(undefined, { cause: signal.reason }));
	const { promise, resolve, reject } = Promise.withResolvers<T>();
	const onAbort = (): void => reject(new ToolAbortError(undefined, { cause: signal.reason }));
...
export const logger = {
	level: (process.env.DSH_BROWSER_LOG ?? "info").toLowerCase() as "debug" | "info" | "warn" | "error",
...
export async function removeWithRetries(target: string, attempts = 8, delayMs = 150): Promise<void> {
	...
	for (const dir of pathVar.split(path.delimiter)) {
		...
		for (const ext of exts) {
			...
			try {
				...
				if (stat.isFile() && (isWin || (stat.mode & 0o111) !== 0)) return candidate;
			} catch {
				// not here
			}
		}
...
export function puppeteerExecutablePathFromEnv(): string | undefined {
