...
function stealthIgnoreDefaultArgs(executablePath: string | undefined): string[] {
	...
	if (isMicrosoftEdgeExecutable(executablePath)) return STEALTH_IGNORE_DEFAULT_ARGS.filter(f => f !== ENABLE_AUTOMATION_FLAG);
...
export async function loadPuppeteerInWorker(): Promise<typeof Puppeteer> {
...
let resolvedChromium: string | undefined | null; // undefined = unchecked; null = not found
...
function systemChromiumCandidates(platform: NodeJS.Platform = process.platform, home = os.homedir()): string[] {
...
// ---------------------------------------------------------------------------
// Stealth: sourceURL stripping + user-agent override + injected patch scripts
// ---------------------------------------------------------------------------
...
function patchSourceUrl(page: Page): void {
	...
	client.send = async (method: string, params?: Record<string, unknown>) => {
		const next = async (payload?: Record<string, unknown>): Promise<unknown> => {
			try {
		...
		if (typeof value !== "string" || !value.includes(PUPPETEER_SOURCE_URL_SUFFIX)) return next(params);
...
export function buildStealthInjectionScript(): string {
	const joint = STEALTH_PATCH_SCRIPTS.map(
		({ src }) => `
	try {
		${src};
	} catch (e) {}
