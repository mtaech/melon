/**
 * Screenshot recompression pipeline. Ported from oh-my-pi `image-resize.ts`
 * with `Bun.Image` replaced by `sharp` (hard dependency; falls back to the
 * original bytes if the native module fails to load at runtime).
 */
...
type SharpFactory = (input?: Buffer | string, options?: import("sharp").SharpOptions) => import("sharp").Sharp;

let sharpAvailable: boolean | undefined;
async function loadSharp(): Promise<SharpFactory | null> {
	if (sharpAvailable === false) return null;
	try {
		const mod = (await import("sharp")) as { default?: SharpFactory } & Record<string, unknown>;
...
export async function resizeImage(
	...
	const encode = async (width: number, height: number, quality: number, fmt: "png" | "jpeg" | "webp"): Promise<{ buffer: Uint8Array; mimeType: string } | null> => {
		try {
			const pipe = sharp(inputBuffer, { failOn: "none" })
				.resize(width, height, { fit: "fill" })
				.removeAlpha();
			let out: Buffer;
			if (fmt === "png") out = await pipe.png({ palette: true, quality }).toBuffer();
			else if (fmt === "jpeg") out = await pipe.jpeg({ quality, mozjpeg: true }).toBuffer();
			else out = await pipe.webp({ quality }).toBuffer();
	...
	const candidates: Array<{ buffer: Uint8Array; mimeType: string }> = [];
	const formats: Array<"png" | "jpeg" | "webp"> = opts.excludeWebP ? ["png", "jpeg"] : ["png", "jpeg", "webp"];
	...
	if (best.buffer.length > opts.maxBytes) {
		for (const quality of [opts.jpegQuality - 10, opts.jpegQuality - 20, opts.jpegQuality - 30]) {
			...
			if (enc && enc.buffer.length < best.buffer.length) best = enc;
		}
	}
...
function passthrough(buffer: Uint8Array, mimeType: string, originalData: string, decodeFailed: boolean): ResizedImage {
	return {
		buffer,
		mimeType,
		originalWidth: 0,
...
export function _sharpAvailableForTest(): boolean | undefined {
