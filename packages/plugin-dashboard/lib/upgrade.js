...
export async function planUpgrade(profile, name, registry, installed, installedCommit) {
    ...
    const base = { name, installedVersion: installed?.version ?? null, installedCommit, currentSpecifier, profileName: profile.name, currentVersionDate: null, latestVersionDate: null };
    const settings = { ...base, targetLabel: "", targetCommit: null, newSpecifier: "", command: "", wouldChange: false };
    if (source === "git") {
...
export async function applyUninstall(profileDir, plan, runner) {
    ...
    try {
        if (plan.inDependencies) {
            ...
            output.push(out);
            if (code !== 0)
                throw new Error(`dsh plugin remove exited ${code}: ${out.trim().split("\n").slice(-8).join("\n")}`);
        }
