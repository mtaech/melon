...
export async function listProfiles() {
    ...
    for (const entry of entries) {
        if (!entry.isDirectory())
            continue;
        if (!existsSync(path.join(root, entry.name, "package.json")))
            continue;
...
export async function readProfileDir(dir, name) {
    ...
    return {
        ...
        dir,
        bundles: Array.isArray(dsh.profile?.bundles) ? dsh.profile.bundles : [],
        dependencies,
        packageJson: data,
    };
}
...
export function sourceOf(specifier) {
    const s = specifier.trim();
    if (!s)
        return "unknown";
    if (s.startsWith("github:") || s.startsWith("git+") || s.startsWith("git:"))
        return "git";
    if (s.startsWith("file:") || s.startsWith("link:") || s.startsWith("workspace:"))
        return "local";
    return "npm";
}
/** Installed package.json metadata from the profile's node_modules. */
export async function readInstalled(profileDir, name) {
 ...
 */
export function readLockCommit(lockText, name) {
