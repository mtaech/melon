...
import { readProfileDir, readInstalled, readLockCommit, isCorePackage, sourceOf, parseGithubSpec } from "./profile.js";
import { planUpgrade, applyUpgrade, planUninstall, applyUninstall } from "./upgrade.js";
...
function resolveProfileDir(explicit) {
    ...
    for (const dir of candidates) {
        try {
            const raw = JSON.parse(readFileSync(path.join(dir, "package.json"), "utf8"));
            if (raw && typeof raw === "object" && "dsh" in raw)
                return dir;
        }
        catch {
...
export function subprocessRunner(sub) {
...
function packageUrl(name, specifier, source) {
    if (source === "git") {
...
async function buildInventory(profile, registry) {
    const lockText = await fs.readFile(path.join(profile.dir, "pnpm-lock.yaml"), "utf8").catch(() => "");
    const names = new Set([...Object.keys(profile.dependencies), ...profile.bundles]);
...
function extractGhToken(output) {
    const token = output.trim();
    if (!token || token.length < 20 || /error|fail/i.test(token))
        return undefined;
    return token;
}
export function apply(ctx, options = {}) {
    ...
    if (!options.registry) {
        runner(["gh", "auth", "token"], { cwd: process.cwd(), timeoutMs: 5000 })
            .then(({ code, output }) => {
            if (code === 0)
                registry.setGithubToken(extractGhToken(output));
        })
            .catch(() => undefined);
    }
    const handler = async (req, res) => {
        try {
            ...
            if (req.method === "POST" && pathname === `${ROUTE_PREFIX}/upgrade`) {
                ...
                const plan = (await planUpgrade(profile, body.name, registry, installed, installedCommit));
                if (!body.apply) {
                    json(res, 200, { plan });
                    return;
                }
            ...
            if (req.method === "POST" && pathname === `${ROUTE_PREFIX}/uninstall`) {
                ...
                const profile = await readProfileDir(profileDir);
                const plan = planUninstall(profile, body.name);
                if (plan.error) {
                    json(res, 200, { plan, applied: false, log: [] });
                    return;
                }
