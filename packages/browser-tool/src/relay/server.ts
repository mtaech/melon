...
interface RelaySocketImpl extends RelaySocket {
...
export async function startRelayServer(options: RelayServerOptions = {}): Promise<RelayServerHandle> {
	...
	const server = http.createServer((req, res) => {
		void handleHttpGet(bridge, req, res, options.token, () => server.address() as { port: number });
	});
	const wss = new WebSocketServer({
		server,
		maxPayload: MAX_PAYLOAD_BYTES,
		verifyClient: (info: { origin: string; req: { url?: string } }) => verifyUpgrade({ origin: info.origin, rawUrl: info.req.url }, options.token),
		perMessageDeflate: false,
	});

	const sockets = new Set<RelaySocketImpl>();

	wss.on("connection", (raw, request) => {
		const connId: number | null = null;
	...
	const keepalive = setInterval(() => {
		for (const sock of sockets) {
			if (sock.raw.readyState === sock.raw.OPEN) sock.raw.ping();
		}
...
function makeSocket(raw: WebSocket, pool: Set<RelaySocketImpl>): RelaySocketImpl {
	const sock: RelaySocketImpl = {
		raw,
		send(text) {
			if (raw.readyState === raw.OPEN) raw.send(text);
		},
		close() {
			try {
				raw.close();
...
function verifyUpgrade(info: { origin: string; rawUrl?: string | null }, token?: string): boolean {
	if (!token) return true;
	if (info.origin && EXTENSION_ORIGIN.test(info.origin)) return true;
	const url = new URL(info.rawUrl ?? "/", "http://localhost");
	return url.searchParams.get("token") === token;
}

function requiresAuth(req: http.IncomingMessage, token?: string): boolean {
