 ...
 */
export function donorCacheRoots(env: NodeJS.ProcessEnv = process.env): string[] {
	...
	for (const root of roots) {
		if (!root || root.length === 0 || seen.has(root)) continue;
		seen.add(root);
		out.push(root);
	}
	return out;
...
function parseBuildDir(dirName: string, platform: string): string | undefined {
	const prefix = `${platform}-`;
	if (!dirName.startsWith(prefix)) return undefined;
 ...
 */
export function findDonorChromium(
	platform: string,
	wantedBuildId: string | undefined,
	roots: string[] = donorCacheRoots(),
): DonorChromium | undefined {
	let newest: DonorChromium | undefined;
	for (const cacheRoot of roots) {
		...
		for (const dirName of dirs) {
			...
			const found: DonorChromium = { executablePath, dirName, buildId, sourceDir, cacheRoot };
			// An exact build match is as good as downloading; take it immediately.
			if (wantedBuildId !== undefined && buildId === wantedBuildId) return found;
			if (newest === undefined || compareBuildIds(buildId, newest.buildId) > 0) newest = found;
		}
	}
	return newest;
}
 ...
 */
export function adoptDonorChromium(donor: DonorChromium, ourCacheRoot: string): string {
	...
	if (fs.existsSync(destExecutable)) return destExecutable;
