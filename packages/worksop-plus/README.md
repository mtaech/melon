...
- **状态一眼可见**：工作区行、分组/分区标题都带状态点——**琥珀=等你确认**（授权 / 计划 / 回答，优先于运行中）、**蓝色=运行中**、**绿色=有未查看的完成**，同类多于一个时带数字；**折叠起来的项目和分区同样显示**，不用展开就知道哪块等你。会话行用系统同源的 `StateDot`（`useSessionPendingInteraction` 是槽位标准 prop）。
